...
exports.addWidget = async (req, res) => {
  ...
  const widget = await db.github_infos_user_widget
    .create({
      id_user: req.user.id,
      username: username
    })
...
exports.changeParams = async (req, res) => {
  ...
  const widget = await db.github_infos_user_widget.findOne({
...
exports.getWidgets = async (req, res) => {
  const widgets = await db.github_infos_user_widget.findAll({
