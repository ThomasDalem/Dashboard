...
exports.addWidget = async (req, res) => {
    ...
    const widget = await db.weather_widget.create({
        id_user: req.user.id,
        city: city,
...
exports.changeParams = async (req, res) => {
    ...
    const widget = await db.weather_widget.findOne({ where: {id_user: req.user.id, id: id} });
...
exports.getWidgets = async (req, res) => {
    const widgets = await db.weather_widget.findAll({ where: {id_user: req.user.id} });
